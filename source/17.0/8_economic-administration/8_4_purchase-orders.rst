...
.. CAPTURA: 8_4_01 — descomentar el figure cuando esté la imagen
   .. figure:: /_static/img/8_economic-administration/8_4_purchase-orders_01_oc.png
      ...
      La orden de compra de un Viaje: la sección con la ruta y las líneas por parada.

El TMS no pide crear la orden de compra al transportista: la genera y la mantiene él. La
:term:`Orden de compra` es la orden de compra estándar de Odoo (``purchase.order``), marcada
como del TMS y vinculada a su Viaje, y con ella el coste del transporte entra en el circuito
de compras, facturación de proveedores y contabilidad sin ningún paso manual.
...
(la tarea programada de tarificación la crea en su siguiente pasada), al pulsar **Tarificar** en
el Viaje, o a más tardar al cerrarlo. Si el transportista es flota propia, no se crea.

**Qué lleva.** El proveedor es el transportista; el origen, el nombre del Viaje. Una **línea de
sección** describe la ruta: fecha, descripción, matrícula, kilómetros y duración. Debajo, las
**líneas de compra** que genera la tarifa: una por cada detalle de tarifa que aplica, con su
...
**Bloquear** y **Desbloquear** del Viaje hacen exactamente eso a mano (ver
:doc:`/17.0/3_functional-architecture/3_2_2_trips` y
:doc:`/17.0/5_operational-flows/5_5_trip-closing`). Una orden de compra facturada, hecha o
...
líneas sueltas o se abonan (8.5).
