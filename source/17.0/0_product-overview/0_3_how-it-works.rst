...
**El Tramo es cada movimiento de la Orden.** Desde dónde hasta dónde. Una Orden puerta a
puerta tiene un Tramo. Una que pasa por un Hub tiene dos: del origen al Hub y del Hub al
...
franja horaria y tiempo de servicio. Es la pieza con la que trabaja el planificador y la que
ve el conductor en el móvil. Cuando varias Órdenes de una misma operativa tienen que cargar o
descargar en el mismo lugar, el mismo día y con franjas horarias compatibles, el sistema las
...
.. important::
   ...
   margen sale solo: por Orden, por Viaje, por cliente y por transportista.
...
.. mermaid::
   ...
   flowchart LR
       ...
       C --> D[Viajes<br/>recursos asignados]
...
1. **Entra la Orden.** A mano, por fichero, por API o por un aviso automático entre sistemas
   (webhook). Si viene en lote, primero pasa por un :term:`Manifiesto`, que la revisa y
   normaliza las direcciones antes de crear nada.
...
3. **Se planifica.** Las Paradas pendientes se agrupan en Viajes: filtrando y seleccionando a
   ...
   franjas horarias, capacidades, horas de conducción y disponibilidad de conductores.
4. **Se ejecuta.** El conductor ve el Viaje en la app, va Parada a Parada, escanea, entrega,
   ...
   la oficina lo ve en pantalla sin llamar a nadie.
...
6. **Se liquida y se factura.** El Viaje calcula el coste con la tarifa del transportista y
   ...
   lista para facturar. Las dos operaciones usan el mismo mecanismo de tarifas, así que el
   margen queda calculado por Orden, por Viaje, por cliente y por transportista.

Lo que pasa por debajo sin que nadie lo pida
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

- **Geolocalización de direcciones**, con control de calidad: una dirección mal ubicada (o,
  si el Proyecto lo exige, sin horario de apertura) se marca y no deja cerrar el Manifiesto
  hasta que alguien la corrige.
- **Trazabilidad** de cada evento, con quién lo hizo y desde dónde.
- **Indicadores KPI** en las listas de Órdenes y Paradas: estado, puntualidad, si hay
  factura, si falta la prueba de entrega (POD).
- **Documentos**: etiquetas de Bultos, hoja de carga del Viaje («Manifiesto de transporte» en
  la interfaz), detalle de la Orden y POD, generados desde el propio registro.
